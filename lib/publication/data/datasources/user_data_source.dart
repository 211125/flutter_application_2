...
abstract class poshLocalDataSource {
  ...
  Future<void> createMap(MapaposhModel user);
...
Future<List<PostModel>> getUsers() async {
  ...
  Future<List<MapaGetModel>> getMap() async {
    try {
      var response = await http.get(Uri.parse('$_baseUrl/gps/user/1/coordinates'));
      if (response.statusCode == 200) {
        List<MapaGetModel> posts = (jsonDecode(response.body) as List)
            .map((post) => MapaGetModel.fromJson(post))
            .toList();
  ...
  Future<void> createMap(MapaposhModel map) async {
    final url = '$_baseUrl/gps/coordinate';

    // Create a multipart request
    var request = http.MultipartRequest('POST', Uri.parse(url));

    // Add text fields
    request.fields['userId'] = map.userId.toString();
